...
- (void)loadData {
    ...
    NSString *qsql = [NSString stringWithFormat:
                      ...
                      TABLE_PROGRAMS,
                      TABLE_PG_CY_LNK,
                      TABLE_PROGRAMS, kPROGRAMS,
                      TABLE_PG_CY_LNK, kPROGRAMS,
                      TABLE_CYCLES,
                      TABLE_CYCLES, kCYCLES,
                      TABLE_PG_CY_LNK, kCYCLES,
                      TABLE_PROGRAMS, cPgName,
                      TABLE_PG_CY_LNK, cCyOrder];
    NSArray *tmp = [DbManager execQuery:qsql];
    NSDictionary *first = nil;
    NSInteger section = -1;
    for (NSDictionary *item in tmp) {
        if(![[first objectForKey:kPROGRAMS] isEqual:[item objectForKey:kPROGRAMS]]) {
            NSMutableArray *sectionItems = [[NSMutableArray alloc] init];
            section++;
            first = item;
            NSString *title = [first objectForKey:cPgName]?[first objectForKey:cPgName]:@" ";
            NSNumber *key = [first objectForKey:kPROGRAMS];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"PgCell" forIndexPath:indexPath];
    ...
    cell.textLabel.text = [item objectForKey:cCyName]?[item objectForKey:cCyName]:@"<EDIT WEEK SCHEDULE LINK>";
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        ...
        if([[[tableItems objectAtIndex:indexPath.section] objectForKey:kItems] count] == 0) {
            ...
            if([orphans count] > 0) {
                ...
                for(NSDictionary *item in orphans) {
                    orphanListStr = [orphanListStr stringByAppendingFormat:@"%@,", [[item objectForKey:kPROGRAMS] stringValue]];
                }
        ...
        [newObj removeObjectForKey:kCYCLES];
        [newObj removeObjectForKey:cCyName];
        //
        ...
        NSString *sqlCmd = [NSString stringWithFormat:
                            ...
                            TABLE_PG_CY_LNK,
                            kPROGRAMS,
                            [Utils dbIdFromInteger:[[newObj objectForKey:kPROGRAMS] integerValue]]];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if([segue.identifier isEqualToString:@"ShowFrm"]) {
        ...
        vc.delegate = self;
        [vc setPlistFileName:@"Programs.plist"];
        vc.navigationItem.title = @"New Program";
        vc.recordData = sender;
        vc.primaryKey = kPROGRAMS;
    } else if([segue.identifier isEqualToString:@"ShowFrm2"]) {
        FrmViewController *vc = [segue destinationViewController];
        vc.delegate = self;
        [vc setPlistFileName:@"Cycles2.plist"];
        vc.navigationItem.title = @"Choose Week Schedule";
        vc.recordData = sender;
        vc.primaryKey = kLINK;
    }
}
...
- (void)FrmViewController:(FrmViewController *)viewController newRecordWithKey:(NSInteger)key intoTable:(NSString *)table {
    if([table isEqualToString:TABLE_PROGRAMS]) {
        NSString *sqlCmd = [NSString stringWithFormat:
