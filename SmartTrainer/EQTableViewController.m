...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"EQCell" forIndexPath:indexPath];
    ...
    cell.textLabel.text = [item objectForKey:cEqName];
    cell.detailTextLabel.textColor = [UIColor darkGrayColor];
    cell.detailTextLabel.text = [NSString stringWithFormat:@"%@ %@",
                                 [[item objectForKey:cEqWeight] stringValue],
                                 [Utils weightUnit]];
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        ...
        [DbManager delUpdItemWithKey:[[obj objectForKey:kEQUIPMENT] integerValue]
                             keyName:kEQUIPMENT
                           fromTable:TABLE_EQUIPMENT
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if([segue.identifier isEqualToString:@"ShowFrm"]) {
        ...
        vc.delegate = self;
        [vc setPlistFileName:@"Equipment.plist"];
        vc.navigationItem.title = (sender)?@"Edit Bar":@"New Bar";
        vc.recordData = sender;
        vc.primaryKey = kEQUIPMENT;
    }
}
...
- (void)filterContentForSearchText:(NSString*)searchText scope:(NSString*)scope
{
    ...
    for(NSDictionary *dataItem in self.tableItems)
    {
        ...
        if([searchText length] > 0) {
            result = [[[dataItem objectForKey:cEqName] uppercaseString] containsString:[searchText uppercaseString]];
        }
